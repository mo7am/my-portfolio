@extends('layout.master')
@section('title', __('dashboard.create_language'))
@section('content')
<div class="container-xxl flex-grow-1 container-p-y">
  @include('partials.page-intro', ['title' => __('dashboard.create_language'), 'description' => __('dashboard.intros.language_form')])
  <div class="card">
    <div class="card-body">
      <form method="POST" action="{{ route('clients.languages.store') }}">
        @csrf
        @include('client.languages.partials.form', ['language' => $language])
        <button type="submit" class="btn btn-primary mt-3">{{ __('app.save') }}</button>
        <a href="{{ route('clients.languages.index') }}" class="btn btn-label-secondary mt-3">{{ __('app.cancel') }}</a>
      </form>
    </div>
  </div>
</div>
@endsection