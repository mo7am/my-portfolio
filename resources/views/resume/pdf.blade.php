...
<body>
  <div class="outer-border">
    <div class="inner-border">
      ...
      <h2 style="text-align:center; border:0; margin-top:5px;text-decoration: underline;font-size:23px">{{ pdf_text($user->name) }}</h2>
      @if($user->job_title)
        <h4 style="text-align:center; border:0;font-size:16px; margin-top:-20px">{{ pdf_text($user->job_title) }}</h4>
      @endif
        ...
        <tr>
          <td class="header-left">
            ...
            @if ($user->marital_status)
              <p><b>{{ pdf_text(__('cv.marital_status')) }}:</b> {{ pdf_text(marital_status_label($user->marital_status)) }}</p>
            @endif
          </td>
          @if(!empty($logoData))
            <td class="header-right">
              <img src="data:image/{{ $logoType }};base64,{{ $logoData }}" alt="{{ pdf_text($user->name) }}">
            </td>
          @endif
        </tr>
      </table>
      ...
      @if (tenant()->is_show_experience && $experiences->count() > 0)
        ...
        @foreach ($experiences as $experience)
          <table style="margin-bottom:-15px;" class="two-col">
            <tr>
              <td class="left">
                <p><b>{{ pdf_text(collect([$experience->title, $experience->company])->filter()->implode(' · ')) }}</b></p>
                ...
                @if($experience->description)
                  <ul><li>{{ pdf_text($experience->description) }}</li></ul>
                @endif
              </td>
              <td class="right">
            ...
            <tr>
              <td class="left">
                <p>
                  ...
                  @if($educational->institution)<br>{{ pdf_text($educational->institution) }}@endif
      ...
      @if (tenant()->is_show_certification && $certifications->count() > 0)
        <h2 style="text-decoration: underline; margin-bottom:-15px;">{{ pdf_text(__('cv.certifications')) }}:</h2>
        @foreach ($certifications as $item)
          <table class="two-col" style="margin-bottom:-15px;">
            <tr>
              <td class="left">
                <p><b>{{ pdf_text($item->title) }}</b>@if($item->issuer) — {{ pdf_text($item->issuer) }}@endif</p>
              </td>
      ...
      @if (tenant()->is_show_course && $courses->count() > 0)
        ...
        @foreach ($courses as $item)
          <table class="two-col" style="margin-bottom:-15px;">
            <tr>
              <td class="left">
                <p><b>{{ pdf_text($item->title) }}</b>@if($item->provider) — {{ pdf_text($item->provider) }}@endif</p>
              </td>
              <td class="right">
        ...
        @foreach ($awards as $item)
          <table class="two-col" style="margin-bottom:-15px;">
            <tr>
              <td class="left">
                <p><b>{{ pdf_text($item->title) }}</b>@if($item->issuer) — {{ pdf_text($item->issuer) }}@endif</p>
                @if($item->description)<p>{{ pdf_text($item->description) }}</p>@endif
              </td>
      ...
      @if (tenant()->is_show_volunteering && $volunteerings->count() > 0)
        <h2 style="text-decoration: underline; margin-bottom:-15px;">{{ pdf_text(__('cv.volunteering')) }}:</h2>
        @foreach ($volunteerings as $item)
          <table class="two-col" style="margin-bottom:-15px;">
            <tr>
              <td class="left">
                <p><b>{{ pdf_text(collect([$item->organization, $item->role])->filter()->implode(' · ')) }}</b></p>
                @if($item->description)<ul><li>{{ pdf_text($item->description) }}</li></ul>@endif
              </td>
              <td class="right">
                @if($item->start_date){{ \Carbon\Carbon::parse($item->start_date)->format('M Y') }}@endif
                @if($item->end_date) – {{ \Carbon\Carbon::parse($item->end_date)->format('M Y') }}
                @elseif($item->start_date) – {{ pdf_text(__('app.present')) }}@endif
              </td>
            </tr>
          </table>
        @endforeach
      ...
      @if (tenant()->is_show_language && $languages->count() > 0)
        ...
        @foreach ($languages as $language)
          <p style="margin-bottom:-10px;"><b>{{ pdf_text($language->language) }}:</b> {{ pdf_text($language->description) }}</p>
        @endforeach
      @endif

      @if (tenant()->is_show_project && $projects->count() > 0)
        <h2 style="text-decoration: underline; margin-bottom: -2px;">{{ pdf_text(__('cv.work_samples')) }}:</h2>
        <ul style="list-style-type: disc; padding-left: 20px;">
          @if($projectGroups->count())
                  ...
                  @foreach ($projects->where('project_work_id', $projectGroup->id) as $project)
                    <li style="font-weight: normal; margin-bottom:5px;">
                      ...
                      @if($project->source_code)
                        <span> > {{ pdf_text(__('app.source')) }}: <a href="{{ $project->source_code }}">{{ $project->source_code }}</a></span><br>
                      @endif
                      @if($project->website_url)
                        <span> > {{ pdf_text(__('app.live_site')) }}: <a href="{{ $project->website_url }}">{{ $project->website_url }}</a></span>
                      @endif
                    </li>
                  @endforeach
                </ul>
            ...
            @foreach ($projects->whereNull('project_work_id') as $project)
              <li style="font-weight: normal; margin-bottom:5px;">
                <span>{{ pdf_text($project->title) }}</span>
                @if($project->source_code)<br>> {{ pdf_text(__('app.source')) }}: {{ $project->source_code }}@endif
                @if($project->website_url)<br>> {{ pdf_text(__('app.live_site')) }}: {{ $project->website_url }}@endif
              </li>
            @endforeach
          @else
            @foreach ($projects as $project)
              <li>
                ...
                @if($project->role) — {{ pdf_text($project->role) }}@endif
                @if($project->description)<br>{{ pdf_text($project->description) }}@endif
              </li>
            @endforeach
          @endif
        </ul>
      ...
      @if (tenant()->is_show_website && $websites->count() > 0)
        <h2 style="text-decoration: underline; margin-bottom:-2px;">{{ pdf_text(__('cv.online_presence')) }}:</h2>
        <ul>
          @foreach ($websites as $website)
            <li>{{ pdf_text($website->name) }} - <a href="{{ $website->url }}">{{ $website->url }}</a></li>
          @endforeach
        </ul>
      @endif
