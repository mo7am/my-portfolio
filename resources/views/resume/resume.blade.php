        ...
        <section class="resume-section personal-info" data-aos="fade-up">
            <div class="info-wrapper">
                <div class="info-text">
                    ...
                    <div class="info-grid">
                        <div class="info-row"><span class="label">{{ __('cv.name') }}</span><span
                                class="value">{{ ucwords($user->name) }}</span></div>
                        <div class="info-row"><span class="label">{{ __('cv.email') }}</span><span class="value"><a
                                    href="mailto:{{ $user->email }}">{{ $user->email }}</a></span></div>
                        @if ($user->phone)
                            <div class="info-row"><span class="label">{{ __('cv.phone') }}</span><span class="value"><a
                                        href="https://wa.me/{{ $user->phone }}" target="_blank"
                                        rel="noopener">{{ $user->phone }}</a></span></div>
                        @endif
                        @if ($user->address)
                            <div class="info-row"><span class="label">{{ __('cv.address') }}</span><span
                                    class="value">{{ $user->address }}</span></div>
                        @endif
                        @if ($user->birthdate)
                            <div class="info-row"><span class="label">{{ __('cv.date_of_birth') }}</span><span
                                    class="value">{{ \Carbon\Carbon::parse($user->birthdate)->locale(app()->getLocale())->translatedFormat('j F Y') }}</span>
                            </div>
                        @endif
                        @if ($user->nationality)
                            <div class="info-row"><span class="label">{{ __('cv.nationality') }}</span><span
                                    class="value">{{ $user->nationality }}</span></div>
                        @endif
                        @if ($user->marital_status)
                            <div class="info-row"><span class="label">{{ __('cv.marital_status') }}</span><span
                                    class="value">{{ marital_status_label($user->marital_status) }}</span></div>
                        @endif
                        @if ($user->job_title)
                            <div class="info-row"><span class="label">{{ __('cv.role') }}</span><span
                                    class="value">{{ ucwords($user->job_title) }}</span></div>
                        @endif
                    </div>
                </div>
                @if ($user->getFirstMediaUrl('logo'))
                    <div class="info-photo">
                        <img class="square-image js-lightbox-trigger" src="{{ $user->getFirstMediaUrl('logo') }}"
                            alt="{{ ucwords($user->name) }}" data-lightbox-src="{{ $user->getFirstMediaUrl('logo') }}"
                            data-lightbox-alt="{{ ucwords($user->name) }}" role="button" tabindex="0"
                    ...
                    @foreach ($experiences as $experience)
                        <li>
                            ...
                            <div class="content new-content">
                                <strong>{{ $experience->title }}@if ($experience->company)
                                        · {{ $experience->company }}
                                    @endif
                                </strong>
                ...
                <ul class="education-list">
                    @foreach ($certifications as $item)
                        <li>
                            ...
                            <span
                                class="period">{{ $item->issued_at ? \Carbon\Carbon::parse($item->issued_at)->format('M Y') : '' }}</span>
                        </li>
                    @endforeach
                </ul>
            </section>
                ...
                <ul class="education-list">
                    @foreach ($awards as $item)
                        <li>
                            ...
                            <span
                                class="period">{{ $item->awarded_at ? \Carbon\Carbon::parse($item->awarded_at)->format('M Y') : '' }}</span>
                        </li>
                    @endforeach
                </ul>
            </section>
                ...
                <ul class="timeline">
                    @foreach ($volunteerings as $item)
                        <li>
                            ...
                            <div class="content">
                                <strong>{{ $item->organization }}@if ($item->role)
                                        · {{ $item->role }}
                                    @endif
                                </strong>
                                @if ($item->description)
            ...
            <section class="resume-section" data-aos="fade-up">
                ...
                <ul class="language-list">
                    @foreach ($languages as $language)
                        <li><span>{{ $language->language }}</span><span class="level">{{ $language->description }}</span>
                        </li>
                    @endforeach
                </ul>
            </section>
                        ...
                        <li>
                            <span>
                                ...
                                @if ($project->description)
                                    <br><span
                                        class="muted">{{ \Illuminate\Support\Str::limit($project->description, 140) }}</span>
                                @endif
                            </span>
                            <span
                                class="period">{{ $project->date ? \Carbon\Carbon::parse($project->date)->format('Y') : '' }}</span>
                        </li>
                    @endforeach
                </ul>
            </section>
            ...
            <section class="resume-section" data-aos="fade-up">
                ...
                <ul class="website-list">
                    @foreach ($websites as $website)
                        <li><span>{{ $website->name }}</span><a href="{{ $website->url }}" target="_blank"
                                rel="noopener">{{ $website->url }}</a></li>
                    @endforeach
                </ul>
            </section>
        @endif
        ...
        @if (tenant()->is_show_download_cv)
            <section class="resume-section resume-download" data-aos="fade-up">
                <div class="resume-download__actions">
                    <a class="btn primary"
                        href="{{ route('portfolio.download', ['domain' => tenant()->user->domain]) }}">
                        <i class="bi bi-download"></i> {{ __('app.download_cv') }}
                    </a>

                    <a class="btn" href="{{ route('portfolio.view-pdf', ['domain' => tenant()->user->domain]) }}"
                        target="_blank" rel="noopener">
                        <i class="bi bi-file-earmark-pdf"></i> {{ __('app.view_cv') }}
                    </a>

                    @if ($driveConfigured ?? false)
                        <button type="button" class="btn" id="shareDriveBtn"
                ...
                <div class="drive-link-box" id="sitePdfLinkBox">
                    ...
                    <div class="drive-link-row">
                        <input type="url" id="sitePdfLinkInput" class="drive-link-input" readonly
                            value="{{ route('portfolio.view-pdf', ['domain' => tenant()->user->domain]) }}">
                        <button type="button" class="btn" id="copySitePdfLinkBtn"
                ...
                <div class="drive-link-box {{ $user->cv_drive_link ? '' : 'is-empty' }}" id="driveLinkBox"
                    @if (!($driveConfigured ?? false) && !$user->cv_drive_link) hidden @endif>
                    <label class="drive-link-label" for="driveLinkInput">{{ __('app.drive_link_label') }}</label>
                    <div class="drive-link-row">
                        <input type="url" id="driveLinkInput" class="drive-link-input" readonly
                            value="{{ $user->cv_drive_link }}" placeholder="{{ __('app.drive_link_placeholder') }}">
                        <button type="button" class="btn" id="copyDriveLinkBtn" @disabled(!$user->cv_drive_link)
                            title="{{ __('app.copy_link') }}">
                            <i class="bi bi-clipboard"></i> {{ __('app.copy_link') }}
                        </button>
                        <a class="btn primary" id="openDriveLinkBtn" href="{{ $user->cv_drive_link ?: '#' }}"
                            target="_blank" rel="noopener" @if (!$user->cv_drive_link) hidden @endif>
